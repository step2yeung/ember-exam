...
module.exports = TestCommand.extend({
  ...
  _addLoadBalancingBrowserSocketEvents(config) {
    ...
    events['tests-start'] = function() {
      log.info(`${this.launcher.settings.test_page}: tests-start event received`);
      if (!init) {
    ...
    events['testem:set-modules-queue'] = function(modules, browserId) {
      log.info(`${this.launcher.settings.test_page}: testem:set-modules-queue event received`);
      if (this.launcher.settings.test_page === undefined) {
        log.info(`${JSON.stringify(this.launcher.settings)}`);
      }
    ...
    events['testem:next-module-request'] = function(browserId) {
      log.info(`[${Date.now()}]${this.launcher.settings.test_page}: testem:next-module-request event received`);
      testemEvents.nextModuleResponse(
        browserId,
        this.socket,
        commands.get('writeExecutionFile'),
        this.launcher.settings.test_page
      );
    };
