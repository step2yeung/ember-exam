...
class TestemEvents {
  ...
  _generatesModuleMapJsonObject(browserCount) {
    ...
    // if there are failed tests, then write this stuff out
    if (output.failedBrowsers.length > 0) {
      output.failedModulesMap = (() => {
        let failedModulesMap = Object.create(null);
        for (const [browserId, moduleList] of this.stateManager.getFailedModulesMap()) {
          failedModulesMap[browserId] = moduleList;
        }

        return failedModulesMap;
      })();
    }
