...
class TestemEvents {
   ...
   */
  nextModuleResponse(browserId, socket, writeExecutionFile, testpage) {
    const moduleQueue =
      this.stateManager.getTestModuleQueue() ||
      this.stateManager.getReplayExecutionModuleQueue(browserId);
    if (!moduleQueue) {
      throw new Error(`No moduleQueue was set. BrowserId:${browserId},   TestPage:${testpage}`);
    }

    const moduleName = moduleQueue.shift();
    log.info(`sending testem:next-module-response event: moduleName -  ${moduleName}`);
    log.info(`to ${testpage}`)
